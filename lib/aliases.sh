...
sr_apply_aliases() {
  if [ ! -f "$SR_ALIASES_JSON" ]; then
    printf "⚠️  aliases.json not found at %s\n" "$SR_ALIASES_JSON" >&2
    return 1
  fi
  ...
  # Robust iteration (base64 rows → jq decode in the loop)
  while IFS= read -r row; do
    ...
    if [ "$key" = "head" ] || [ "$key" = "tail" ] || [ "$key" = "less" ]; then
      printf "# skipped: alias %s → %s%s (compatibility)\n" "$key" "$repl" "${flags:+ $flags}" >> "$SR_ALIASES_OUT"
      continue
    fi
    ...
    if _replacement_available "$repl"; then
      ...
      if [ "$key" = "cat" ]; then
        if [ -n "${VSCODE_SHELL_INTEGRATION:-}" ]; then
          printf "# skipped: alias cat → %s (VS Code shell integration detected)\n" "$repl" >> "$SR_ALIASES_OUT"
        else
      ...
      printf "# skipped: alias %s → %s (missing on PATH)\n" "$key" "$repl" >> "$SR_ALIASES_OUT"
    fi
...

  {
    ...
    jq -r 'to_entries[] | "# " + .key + " → " + .value.replacement
      + (if .value.flags|length>0 then " " + .value.flags else "" end)
      + " — " + .value.reasoning' "$SR_ALIASES_JSON"
  } >> "$SR_ALIASES_OUT"

  _add_sourcing_line "$HOME/.zshrc"
  _add_sourcing_line "$HOME/.bashrc"

  printf "✅ Aliases generated: %s\n" "$SR_ALIASES_OUT"
  printf "ℹ️  Sourced from ~/.zshrc (and ~/.bashrc if present). Restart shell or: source %s\n" "$SR_ALIASES_OUT"
}

sr_remove_aliases() {
  _remove_sourcing_line "$HOME/.zshrc"
  _remove_sourcing_line "$HOME/.bashrc"
  printf "✅ Removed sourcing lines from rc files. Alias file left at: %s\n" "$SR_ALIASES_OUT"
}
...
sr_aliases_status() {
  ...
  header="STATUS  LEGACY  →  REPLACEMENT  FLAGS"
  ...
  for r in "${ROWS[@]}"; do
    ...
    line=$(printf '%-3s  %-6s  →  %-12s  %s' "$mark" "$legacy" "$repl" "$flags")
    if [ $have_gum -eq 1 ]; then
      if [ "$status" = "OK" ]; then
